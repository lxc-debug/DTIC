...
subprocess.run(shlex.split(f'python main.py --lr {0.001} --weight_decay {0.0001}   --logdir {"./log_tmp/test_save/debug_base_four.log"}  --use_list {"leader_four"} --test --process_data --use_base'))
...
for weight_decay in weight_decay_li:
    for lr in lr_li:
        try:
            subprocess.run(shlex.split(f'python main.py --lr {lr} --weight_decay {weight_decay}   --logdir {"./log_tmp/test_save/base_four_noarchi.log"}  --use_list {"leader_four"} --test --use_base'),check=True)
        except subprocess.CalledProcessError:
            file_logger.error('something wrong in run.py')