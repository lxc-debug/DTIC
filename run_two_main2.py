...
for weight_decay in weight_decay_li:
    for lr in lr_li:
        try:
            subprocess.run(shlex.split(f'python main.py --lr {lr} --weight_decay {weight_decay}   --logdir {"./log_tmp/main/base_two_2.log"}  --use_list {"leader_two"} --main --use_base --use_q_node --process_data --gin_layer {3}'),check=True)
        except subprocess.CalledProcessError:
            file_logger.error('something wrong in run.py')
